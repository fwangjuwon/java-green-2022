...
public class CoronaTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String sql = "INSERT INTO userTbl(addr, mgtStaDd, pcrPsblYn, ratPsblYn, recuClCd, rprtWorpClicFndtTgtYn, sgguCdNm, sidoCdNm, telno, XPos, XPosWgs84, YPos, YPosWgs84, yadmNm, ykihoEnc) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            PreparedStatement pstmt = conndb.prepareStatement(sql);

            for (int i = 0; i < result.size(); i++) {
                pstmt.setString(1, result.get(i).getAddr());
                pstmt.setLong(2, result.get(i).getMgtStaDd());
                pstmt.setString(3, result.get(i).getPcrPsblYn());
                pstmt.setString(4, result.get(i).getRatPsblYn());
                pstmt.setLong(5, result.get(i).getRecuClCd());
                ...
                pstmt.setLong(10, result.get(i).getXPos());
                pstmt.setString(11, result.get(i).getXPosWgs84());
                pstmt.setLong(12, result.get(i).getYPos());
