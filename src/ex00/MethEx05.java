...
//attack으로 다 바꿈 > 다 된다. 타입(매개변수)의 개수나 위치가 다르면 다른 함수(메소드)로 인식하게 된다.  = 오버로딩
//코드의 가독성이 좋아지고 편해진다. 
// 다른 타입으로만 넣어주면 된다. 
//1. method는 동일한 이름으로 만들 수 없다.
//2. 단, 매개변수의 타입이 다르거나 위치가 다르거나 개수가 다르면 다른 매서드로 인식한다.
//3. 장점: 개발자 입장에서 함수 이름을 하나만 기억하면 된다.
//4. 단점: 경우의 수가 너무 많아지면 퇴사하고 싶다. 너무 많은 메서드가 필요하다> 한계!!
//5. 어떨 때 사용하냐? 개수가 적을 때! <20개 ~ 10개 안
...
public class MethEx05 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(1); // 매개변수 int
        System.out.println(1.0); // 매개변수 double
        System.out.println(true); // 매개변수 boolean 지금 이 세개를 다른 함수로 인식하고 있다. > 우리 입장에선 argument만 바꾸면 된다.
    }
}