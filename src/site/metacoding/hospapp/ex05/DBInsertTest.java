...
public class DBInsertTest {
    ...
    // KISS하게 만드는 것이 좋다. (KEEP IT SMALL AND SIMPLE)
    // totalcount기반으로 전체 데이터 다운로드 해서 자바 오브젝트로 파싱하기
    ...
    public static List<Hospital> migration() {
        ...
        for (Item item : list) {
            ...
            hs.copy(item); // item 통째로 hs에 넣는다. copy method를 내가 만든다. -> hospital class에서 만듦
            hospitals.add(hs);
        }
    ...
    public static void main(String[] args) {
        try {
            ...
            for (Hospital hospital : hospitals) {
                ...
                // 문제 되는 코드 -> 한방에 insert문을 전송해서 한번에 commit !! 계속 for문 돌려야하니까 5682번가량 i/o가 생긴다.
                // = 그러므로 배치프로그램을 돌린다 !! batch program(일괄처리!)
                // pstmt.executeUpdate();

                pstmt.addBatch(); // 버퍼에 담기(항아리)
                pstmt.clearParameters(); // 완성된 쿼리를 원복: pstmt를 원복 (이유 pstmt한개만 쓸거니까 )
            }
            pstmt.executeBatch(); // 항아리에 담긴걸 한방에 전송
            // pstmt.close();// close해줘야된다. (통신을 빨리 차단해버림)
